...
|    -O2    |  180.55 | 67.8 |
|    -O3    |  46.06  | 17,1 |
|-03 -fno-unroll-loops | 39.72 | 15.2 |
...
`SIMD имеет вид: интрукция dst src1 src2`

В первом случае перед выполнением одной `SIMD` инструкции компилятор использует в качестве третьего операнда память и перед вызовом заполняет её нужными значениями. Во втором случае `SIMD`инструкция раскрывается в одну ассемблерную инструкцию.
